...
if(what == "ParkinsonsUCI"){
	trueModelX = expression({
		fileName = "ParkinsonsUCI"
		dat=read.csv("ParkinsonsUCI.txt")
		X= dat[,!colnames(dat)%in%c("name", "status")]
	}) 
	
	trueModelY = expression({
		Y = as.data.frame(as.factor(dat$status))
	})
			...
			for(pSub in p:1){
				...
				if(defaultMtry) {
					...
					if(meth=="Strobl"){
						...
						RF = TrainForest(dat, RFMtry, nTree, forestType, corr.threshold=corr.thresh, maxLevel=floor(log2(0.368*nPts/minNumPtsPerPart)))
					} else{
						RF = TrainForest(dat, RFMtry, nTree, forestType)
					}
				...
				if(pSub>1){
					...
					if(!defaultMtry){
						if(meth=="Strobl"){
							RF = TrainForest(dat, RFMtry, nTree, forestType, corr.threshold=-0.1, maxLevel=floor(0.368*nPts/minNumPtsPerPart))
						} else{
							RF = TrainForest(dat, RFMtry, nTree, forestType)  ### Jul 19: is this gonna work? cfr xVarIdx below
						}
					}	
						# pointless to retrain the same model if no cross validation is performed
					currRawImp = Importance(RF, forestType)
					# note this is actually conditional importance if "meth=='Strobl'"
	
					if(pSub==p){
						nonRecMargImp = unname(currRawImp)
						if(meth%in%c("Marginal", "Strobl") | doWarmStart){
							# 
							combImp = nonRecMargImp # this is initialized when trying all p variables -- for mu = Inf, currRawImp and currCondImp are irrelevant
													...
													# obtained below with a call to varimpGGG for my method
						}								
					}
					
					if(meth=="ClusterSimple" & !doWarmStart) {
					...
					if(!meth%in%c("Marginal", "Strobl") & !doWarmStart){
						...
						ptm <- proc.time()
							...
							if(includeSeed)
								set.seed(pSub)
							currCondImp = varimpGGG(object=RF, forestType = forestType, dat = dat, clusterPart = T, minNumPtsPerPart=minNumPtsPerPart, 
						...
						#	currCondImp = importance(RF, scale=F)[, 1]
						#}
						ptm = round((proc.time() - ptm)/60)
						cat(paste("The computation of conditional importance took", unname(summary(ptm)[3]), "minutes\n"))
					} else{
						if(meth=="Strobl")
							cat("Strobl-conditional importance computation completed\n")
						else	
							cat("Marginal importance computation completed\n")
						currCondImp = 0
					}	
					if(!meth%in%c("Marginal", "Strobl") & !doWarmStart){
						# this will be defined only starting when doWarmStart becames FALSE
						#combImp  = mu*currRawImp+(1-mu)*currCondImp
						combImp  = currCondImp #### the actual value of mu is ignored (no convex combinations)
					} # else it uses the combImp initialized in the beginning, minus the components corresponding to variables already eliminated
					worstVar = which.min(combImp)
				} else{
					# from if (pSub>1)
					currRawImp = currCondImp = combImp = NULL
				}
		
				resSub[[pSub]] = list(variablesIn = variablesIn, perfMean = RFPerfMean, perfSE = RFPerfSE) # combImp = unname(combImp), margImp=unname(currRawImp), condImp = unname(currCondImp) # commented out to save memory
				...
				if(pSub>1){
					...
					if(meth%in%c("Marginal", "Strobl") | doWarmStart){
						combImp = combImp[-worstVar] # if doWarmStart is TRUE, this will be reduced until doWarmStart becomes FALSE
					}
				}	
			}
